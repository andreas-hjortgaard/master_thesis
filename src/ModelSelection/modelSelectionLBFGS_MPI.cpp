...
#include "ObjectiveFunctions/LogLikelihoodGradient_MPI.h"

#include "Learning/LBFGS_MPI.h"
...
void usage() {
  cout << "modelSelectionLBFGS [rootpath] [object] [stepSize] [lambda] [kickstart] [weightpath]" << endl;
  cout << "Options:" << endl;
...
int main(int argc, char **argv) {
  ...
  int id, worldSize, namelen;
  MPI::Init(argc, argv);
  id = MPI::COMM_WORLD.Get_rank();
  worldSize = MPI::COMM_WORLD.Get_size();
  char processor_name[MPI_MAX_PROCESSOR_NAME+1];
  MPI_Get_processor_name(processor_name, &namelen);
  cout << "Process " << id << " of " << worldSize << " on " << processor_name << endl;
  ...
  try {
    ...
    if (e == FILE_NOT_FOUND) {
      if (id == 0) cerr << "There was a problem opening a file!" << endl;
      return -1;
    }
  }
  ...
  LogLikelihoodGradient_MPI loglikgrad(&datamanTrain, &crf);
  ...
  ostringstream os;
  os << rootPath << "/results/lbfgs/" << object << "_" << stepSize << "_" << lambda << "_info.txt";
  string infoFile = os.str();
 
  os.clear();
  os.str("");
  os << rootPath << "/results/lbfgs/" << object << "_" << stepSize << "_" << lambda << "_weights.txt";
  string weightFile = os.str();
  
  os.clear();
  os.str("");
  os << rootPath << "/results/lbfgs/" << object << "_" << stepSize << "_" << lambda << "_tempWeights.txt";
  string tempWeightFile = os.str();
 
  // recall overlap figures
  os.clear();
  os.str("");
  os << rootPath << "/results/lbfgs/" << object << "_" << stepSize << "_" << lambda << "_recallOverlapTrain";
  string lossTrain = os.str();
 
  os.clear();
  os.str("");
  os << rootPath << "/results/lbfgs/" << object << "_" << stepSize << "_" << lambda << "_recallOverlapVal";
  string lossVal = os.str();

  ofstream infostream;
  // print info for model selection run
  if (id == 0) {
    infostream.open(infoFile.c_str());
    infostream << "Learning algorithm      : LBFGS\n";
    infostream << "Objective function      : LogLikelihood\n";
    infostream << "Gradient                : LogLikelihoodGradient\n";
    infostream << "Object                  : " << object << "\n";
    infostream << "Step size               : " << stepSize << "\n";
    infostream << "Lambda                  : " << lambda << "\n";
    infostream << "Kickstart               : " << kickstart << " " << initWPath << "\n";
    infostream << "Number of processes     : " << worldSize << "\n";
    infostream.close();
  }
  ...
  if (kickstart) {
    ...
    for (int i=0; i<weightDim; i++) {
      initialW[i] = 0.0 + i*0.00001; //(double) rand() / RAND_MAX)*0.2 - 0.1;
    }
    crf.setWeights(initialW);
  }

  // create learning algorithm
  LBFGS_MPI lbfgs(&loglik, &loglikgrad);
  ...
  //TODO: parallelize the computation of recall-overlap
  if (id == 0) {
