...
void usage() {
  cout << "cornerMarginalsPiecewise [rootpath] [object] [train/val/test] [imageNumber] [method] [weightsPath]" << endl;
}
...
int main(int argc, char **argv) {
  ...
  ostringstream os;
  os << rootPath << "/results/cornerMarginals/" << object << "_" << partition << "_imageNumber_" << imageNumber << "_" << method << "_stepSize_" << stepSize << "_width_" << iiWidth-1 << "_height_" << iiHeight-1 << "_piecewiseLT.txt";
  string distFileLT = os.str();
  
  os.clear();
  os.str("");
  os << rootPath << "/results/cornerMarginals/" << object << "_" << partition << "_imageNumber_" << imageNumber << "_" << method << "_stepSize_" << stepSize << "_width_" << iiWidth-1 << "_height_" << iiHeight-1 << "_piecewiseLB.txt";
  string distFileLB = os.str();
  
  os.clear();
  os.str("");
  os << rootPath << "/results/cornerMarginals/" << object << "_" << partition << "_imageNumber_" << imageNumber << "_" << method << "_stepSize_" << stepSize << "_width_" << iiWidth-1 << "_height_" << iiHeight-1 << "_piecewiseRT.txt";
  string distFileRT = os.str();
  
  os.clear();
  os.str("");
  os << rootPath << "/results/cornerMarginals/" << object << "_" << partition << "_imageNumber_" << imageNumber << "_" << method << "_stepSize_" << stepSize << "_width_" << iiWidth-1 << "_height_" << iiHeight-1 << "_piecewiseRB.txt";
  ...
  for (int y = 0; y < iiHeight-1; y++) {
    ...
    for (int x = 0; x < iiWidth-1; x++) {
      bbox.ltrb[LEFT] = x;
      cornerDist[y*(iiWidth-1)+x] = crf.cornerP(LEFT, TOP, bbox, imageNumber, weights, false, logZ_F);
    }
  }
  ...
  for (int y = 0; y < iiHeight-1; y++) {
    ...
    for (int x = 0; x < iiWidth-1; x++) {
      bbox.ltrb[LEFT] = x;
      cornerDist[y*(iiWidth-1)+x] = crf.cornerP(LEFT, BOTTOM, bbox, imageNumber, weights, false, logZ_F);
    }
  }
  stopComputeCorner = gettime();
  cout << "... in " << stopComputeCorner-startComputeCorner << " seconds" << endl;
  ...
  for (int y = 0; y < iiHeight-1; y++) {
    ...
    for (int x = 0; x < iiWidth-1; x++) {
      bbox.ltrb[RIGHT] = x;
      cornerDist[y*(iiWidth-1)+x] = crf.cornerP(RIGHT, TOP, bbox, imageNumber, weights, false, logZ_F);
    }
  }
  stopComputeCorner = gettime();
  cout << "... in " << stopComputeCorner-startComputeCorner << " seconds" << endl;
  ...
  for (int y = 0; y < iiHeight-1; y++) {
    ...
    for (int x = 0; x < iiWidth-1; x++) {
      bbox.ltrb[RIGHT] = x;
      cornerDist[y*(iiWidth-1)+x] = crf.cornerP(RIGHT, BOTTOM, bbox, imageNumber, weights, false, logZ_F);
    }
  }
  stopComputeCorner = gettime();
  cout << "... in " << stopComputeCorner-startComputeCorner << " seconds" << endl;
